...
### Fixed
- **Critical**: Fixed cross-platform base64 encoding inconsistency causing bundle/check failures between macOS and Linux (GitHub Actions)
  - Replaced deprecated 'binary' encoding with 'utf8' encoding in all Buffer operations
  - Affects: `bundle`, `check`, `batch-check`, `batch-bundle`, `batch-upload`, and `new` commands
  - **Migration**: Existing lens bundles must be re-bundled with `bundle -u` or `batch-bundle` after updating

### Added
- Comprehensive documentation for platform encoding fix (`PLATFORM_ENCODING_FIX.md`)
- Detailed explanation of 3+ possible causes for macOS vs GitHub Actions discrepancies
